  ...
  module Start
    class << self
      def exec?(args)
        ...
        if env_config['construction'].key('hard_setup')
          return false unless hard_setup(vagrant, env_config)
        end

        manual_setting(env_dir, env_config) if env_config['construction'].key?('prepare')
      ...
      def hard_setup(vagrant, env_config)
        env_config['construction']['hard_setup']['msg'].each do |msg|
      ...
      def manual_setting(env_dir, env_config)
        ...
        env_config['construction']['prepare']['msg'].each.with_index(3) do |msg, i|
          Print.command("#{i}. #{msg}")
        end
      end
    end
