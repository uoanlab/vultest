        ...
        module Source
          class PHP < Base
            ...
            def create_tasks
              ...
              FileUtils.cp_r(
                './data/ansible/roles/source/php/tasks/main.yml',
                "#{role_dir}/php/tasks/main.yml"
              )
            end

            def create_vars
              FileUtils.mkdir_p("#{role_dir}/php/vars")
              File.open("#{role_dir}/php/vars/main.yml", 'w') do |vars_file|
                vars_file.puts('---')
                vars_file.puts("version: #{software['version']}")
                vars_file.puts(software_path('/usr/local/bin/php'))
