...
module Ansible
  module Role
    module Content
      module Software
        module Source
          class PCRE < Software::Base
            private

            def create_tasks
              FileUtils.mkdir_p("#{role_dir}/pcre/tasks")
              FileUtils.cp_r(
                './data/ansible/roles/source/pcre/tasks/main.yml',
                "#{role_dir}/pcre/tasks/main.yml"
              )
            end

            def create_vars
              FileUtils.mkdir_p("#{role_dir}/pcre/vars")
              File.open("#{role_dir}/pcre/vars/main.yml", 'w') do |vars_file|
                vars_file.puts('---')
            ...
            def source_path
              ...
              'source_path: ' << if path.nil? || !path.key?('prefix') then '/usr/local/share/doc/pcre'
