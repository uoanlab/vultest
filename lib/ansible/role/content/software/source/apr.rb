        ...
        module Source
          class APR < Base
            ...
            def create_tasks
              ...
              FileUtils.cp_r(
                './data/ansible/roles/source/apr/tasks/main.yml',
                "#{role_dir}/apr/tasks/main.yml"
              )
            end

            def create_vars
              FileUtils.mkdir_p("#{role_dir}/apr/vars")
              File.open("#{role_dir}/apr/vars/main.yml", 'w') do |vars_file|
                vars_file.puts('---')
                vars_file.puts("version: #{software['version']}")
                vars_file.puts(software_path('/usr/local/apr'))
