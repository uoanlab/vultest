      ...
      module PCRE
        class << self
          ...
          def create_tasks(role_dir)
            ...
            erb = ERB.new(
              File.read("#{ANSIBLE_ROLES_TEMPLATE_PATH}/pcre/tasks/main.yml.erb"),
              trim_mode: 2
            )

            File.open("#{role_dir}/pcre/tasks/main.yml", 'w') do |f|
          ...
          def create_vars(role_dir, software)
            ...
            erb = ERB.new(
              File.read("#{ANSIBLE_ROLES_TEMPLATE_PATH}/pcre/vars/main.yml.erb"),
              trim_mode: 2
            )
