      ...
      module APRUtil
        class << self
          ...
          def create_tasks(role_dir)
            ...
            erb = ERB.new(
              File.read("#{ANSIBLE_ROLES_TEMPLATE_PATH}/apr-util/tasks/main.yml.erb"),
              trim_mode: 2
            )

            File.open("#{role_dir}/apr-util/tasks/main.yml", 'w') do |f|
          ...
          def create_vars(role_dir, software)
            ...
            erb = ERB.new(
              File.read("#{ANSIBLE_ROLES_TEMPLATE_PATH}/apr-util/vars/main.yml.erb"),
              trim_mode: 2
            )
            ...
            path =
              if configure.match(/prefix=(.*)/).nil? then '/usr/local/apr'
              else configure.match(/prefix=(.*)/)[1].split(' ')[0]
              end

            File.open("#{role_dir}/apr-util/vars/main.yml", 'w') do |f|
              f.puts(erb.result(binding))
            end
          end
        end
