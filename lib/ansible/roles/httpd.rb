      ...
      module Httpd
        class << self
          ...
          def create_tasks(role_dir, software)
            ...
            erb = ERB.new(
              File.read("#{ANSIBLE_ROLES_TEMPLATE_PATH}/httpd/tasks/main.yml.erb"),
              trim_mode: 2
            )

            File.open("#{role_dir}/httpd/tasks/main.yml", 'w') do |f|
          ...
          def create_vars(role_dir, software)
            ...
            erb = ERB.new(
              File.read("#{ANSIBLE_ROLES_TEMPLATE_PATH}/httpd/vars/main.yml.erb"),
              trim_mode: 2
            )

            version = software['version']
            src_dir = software.fetch('src_dir', '/usr/local/src')
            software_path = SourceInstall.create_software_path(software, '/usr/local/apache2')
            configure_command = SourceInstall.create_configure_command(software)
            perl_path = software.fetch('perl_path', nil)

            File.open("#{role_dir}/httpd/vars/main.yml", 'w') do |f|
              f.puts(erb.result(binding))
            end
          end
        end
