      ...
      module WPCLI
        class << self
          ...
          def create_tasks(role_dir)
            ...
            erb = ERB.new(
              File.read("#{ANSIBLE_ROLES_TEMPLATE_PATH}/wp-cli/tasks/main.yml.erb"),
              trim_mode: 2
            )

            File.open("#{role_dir}/wp-cli/tasks/main.yml", 'w') do |f|
          ...
          def create_vars(role_dir, software)
            ...
            erb = ERB.new(
              File.read("#{ANSIBLE_ROLES_TEMPLATE_PATH}/wp-cli/vars/main.yml.erb"),
              trim_mode: 2
            )

            version = software['version']
            src_dir = software.fetch('src_dir', '/usr/local/src')
            File.open("#{role_dir}/wp-cli/vars/main.yml", 'w') do |f|
              f.puts(erb.result(binding))
            end
          end
        end
